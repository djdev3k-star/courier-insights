...
def load_data():
  """Load and prepare all data for analysis - called once via get_data()"""
  import time
  start = time.time()
  print(f"[DEBUG] Starting data load...")
  
  # Load raw data - only load what's needed
  try:
    print(f"[DEBUG] Loading transactions CSV...")
    transactions = safe_read('reports/monthly_comprehensive/all_transactions_detailed.csv')
    print(f"[DEBUG] Transactions loaded: {len(transactions)} rows in {time.time()-start:.2f}s")
    print(f"[DEBUG] Loading other CSVs...")
    ...
    print(f"[DEBUG] All CSVs loaded in {time.time()-start:.2f}s")
  except Exception as e:
  ...
  # Parse dates (optimized - only critical columns)
  transactions['Trip drop off time'] = pd.to_datetime(transactions['Trip drop off time'], errors='coerce')
  print(f"[DEBUG] Dates parsed in {time.time()-start:.2f}s")
  ...
  print(f"[DEBUG] Location extraction done in {time.time()-start:.2f}s")
  
  # Normalize text fields (optimized - only key columns)
  print(f"[DEBUG] Normalizing text...")
  text_cols = ['Pickup City', 'Dropoff City', 'Restaurant', 'Pickup address', 'Drop off address']
  ...
  transactions['Pickup Area'] = transactions['Pickup City'] + ' ' + transactions['Pickup Zip']
  transactions['Dropoff Area'] = transactions['Dropoff City'] + ' ' + transactions['Dropoff Zip']
  print(f"[DEBUG] Data load complete in {time.time()-start:.2f}s")
